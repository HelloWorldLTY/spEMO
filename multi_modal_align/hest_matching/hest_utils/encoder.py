...
from .encoder_wrappers import TimmCNNEncoder, HFViTEncoder, CLIPVisionModelPostProcessor, DenseNetBackbone, GigapathSlide, ctranspath
...
def build_model(config):
    ...
    if config['loader'] == 'timm_wrapper_cnn':
        ...
        state_dict = torch.load(ckpt_path, weights_only=False)['model']
    ...
    elif config['loader'] == 'gpfm':
        from .GPFM_utils import build_model, build_transform

        model = build_model("GPFM", config["checkpoint_path"])
    ...
    if load_state_dict:
        ...
        missing, unexpected = model.load_state_dict(torch.load(ckpt_path, map_location="cpu", weights_only=False), strict=strict)
